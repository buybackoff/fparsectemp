...
let equals expectedValue r =
    match r with
    | Success struct (v, _, _) when v = expectedValue -> ()
    | Success struct (v, _, _)     -> failwith "Math is hard, let's go shopping!"
    | Failure struct (msg, err, _) -> printf "%s" msg; failwith msg
